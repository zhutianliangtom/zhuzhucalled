...
// 前台状态追踪（由 App.vue 或页面设置）
let _appInForeground = false
let _activeChatUserId = null

export const notification = {
  // 设置前台状态
  setForeground(inForeground) {
  ...
  // 检查是否在查看该用户的聊天页面
  isViewingChat(userId) {
    return _appInForeground && _activeChatUserId === userId
  },
  ...
  showMessageNotification(userId, userName, content, avatarUrl = '', messageId = '') {
    ...
    try {
      ...
      // 检查是否在查看该用户的聊天页面
      if (this.isViewingChat(userId)) {
        console.log('[通知] 正在查看该用户聊天，跳过系统通知')
        return
      }
      ...
      const Build = plus.android.importClass('android.os.Build')
      const Date = plus.android.importClass('java.util.Date')
      ...
      // 创建点击通知后的 Intent
      const openAppIntent = new Intent()
      openAppIntent.setClassName(main, main.getPackageName() + '/.pages.message.chat')
      openAppIntent.putExtra('userId', String(userId))
      openAppIntent.putExtra('userName', displayName)
      ...
      builder.setSmallIcon(plus.runtime.icon) // 使用应用图标
      ...
      builder.setTicker(displayContent)
      ...
      // 使用递增的 notificationId 避免覆盖
      notificationCounter++
      const notificationId = notificationCounter + 10000
